...
faq:
  ...
  - q: "In what order are allow and block decisions made?"
    a: "Allowlist always wins first, then your custom blocklist, then the synced threat-feed cache, and finally a clean allow. So an explicit allowlist entry overrides every feed — use it to rescue a false positive without disabling a whole feed."
...
For every DNS lookup a managed device makes, its resolver asks the API for a verdict. The API checks the org's allowlist, blocklist, and synced threat feeds, returns allow/block, and (optionally) logs the query. Blocking is at the **resolution** layer — a blocked domain simply fails to resolve.
...
```
   ...
   3. threat-FEED cache   ── match → BLOCK  (reason: threat_feed + feed name)
   4. clean               ──────── → ALLOW  (reason: clean)
        │
...
4. Decide **Log queries**. On = visibility into blocks and their reasons; off = no resolution history retained.
...
| `log_queries` | boolean | Records `dns_query_log` for visibility into blocks |
...
**Query-log reason codes:** `allowlisted`, `custom_blocklist`, `threat_feed` (the matching feed is returned in a separate `feed` field), `clean`, and `dns_filter_disabled` when the policy is off.
...
- **Query dashboard empty** → `log_queries` must be on; the dashboard reads `dns_query_log`.
