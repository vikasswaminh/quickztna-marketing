...
Free SSH (remote shell) is now available on every plan, including Free. The 2026 lean pivot went the other way on workforce monitoring: DLP content scanning, CASB, software inventory, user-risk scoring, session recording, remote desktop and the AI Operator were all **removed** from the product; DNS threat filtering was **removed** in September 2026 as well (MagicDNS stays). What replaced them is depth in access governance — JIT access requests with approvals, access-review campaigns, policy version rollback, and signed compliance evidence — plus DNS threat filtering and a per-org edge firewall. Details on the [features page](/features/).
