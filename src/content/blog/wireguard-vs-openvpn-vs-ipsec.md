...
No native post-quantum key exchange. The protocol supports an optional pre-shared key (PSK) that can be populated from a higher-layer post-quantum exchange. This is the approach used by products that do implement post-quantum key exchange — NetBird and some Tailscale configurations among them. QuickZTNA is not one of them: its tunnels are classical WireGuard (see [ML-KEM-768 explained](/blog/ml-kem-768-explained) for how the construction works).
